...
["", "levenstein.dart", , Y, {
  "^": "",
  main: [function() {
    var t1 = J.get$onClick$x(document.querySelector("#run"));
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(Y.levensteinRun$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
  }, "call$0", "main$closure", 0, 0, 1],
  levensteinRun: [function($event) {
    var source, target, sourceIndex, t1, matrix, distance;
    source = H.interceptedTypeCast(document.querySelector("#string1"), "$isInputElement").value;
    target = H.interceptedTypeCast(document.querySelector("#string2"), "$isInputElement").value;
    sourceIndex = source.length - 1;
    t1 = target.length;
    matrix = Y.levensteinMatrix(source, target);
    P.print("computed " + H.S(matrix));
    if (sourceIndex < 0 || sourceIndex >= matrix.length)
      return H.ioore(matrix, sourceIndex);
    distance = J.$index$asx(matrix[sourceIndex], t1 - 1);
    document.querySelector("#result").textContent = J.toString$0(distance);
    P.print("result " + H.S(distance));
  }, "call$1", "levensteinRun$closure", 2, 0, 6],
  levensteinMatrix: function(source, target) {
    var t1, t2, matrix, i, a, i0, j, line, b, j0, t3, t4, cost;
    t1 = source.length;
    t2 = target.length;
    matrix = Y.levelsteinPrepareMatrix(t1, t2);
    for (i = 0; i < t1; i = i0) {
      a = source[i];
      for (i0 = i + 1, j = 0; j < t2; j = j0) {
        line = "computing " + i + ":" + t1 + "  " + j + ":" + t2;
        H.printString(line);
        b = target[j];
        if (i >= matrix.length)
          return H.ioore(matrix, i);
        j0 = j + 1;
        t3 = J.$add$ns(J.$index$asx(matrix[i], j0), 1);
        if (i0 >= matrix.length)
          return H.ioore(matrix, i0);
        t3 = P.min(t3, J.$add$ns(J.$index$asx(matrix[i0], j), 1));
        if (i >= matrix.length)
          return H.ioore(matrix, i);
        t4 = J.$index$asx(matrix[i], j);
        cost = P.min(t3, J.$add$ns(t4, a === b ? 0 : 1));
        if (i0 >= matrix.length)
          return H.ioore(matrix, i0);
        J.$indexSet$ax(matrix[i0], j0, cost);
      }
    }
    return matrix;
  },
  levelsteinPrepareMatrix: function(sourceLength, targetLength) {
    var matrix, i, costList, t1, j, line;
    matrix = [];
    for (i = 0; i <= sourceLength; ++i) {
      costList = [];
      for (t1 = i === 0, j = 0; j <= targetLength; ++j)
        if (t1) {
          costList.push(j);
          line = "i " + i + " " + j;
          H.printString(line);
      ...
      matrix.push(costList);
    }
    P.print("prepared " + H.S(matrix));
    return matrix;
  }
},
1],
